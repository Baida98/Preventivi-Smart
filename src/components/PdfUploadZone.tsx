...
interface PdfUploadZoneProps {
  ...
  onCategoryDetected?: (categoryId: string) => void;
  onDismiss: () => void;
  userId?: string;
}
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (pipelineResult.success && pipelineResult.quote) {
        const hasAI = llmKeys.hasToken();
        toast.success(
          `Preventivo estratto · Confidenza ${pipelineResult.confidence.toFixed(0)}%${hasAI ? ' · AI ✓' : ''}`
        );
        ...
        // Notifica la categoria rilevata dall'AI
        if (pipelineResult.detectedCategory && onCategoryDetected) {
          onCategoryDetected(pipelineResult.detectedCategory);
        }
        ...
        toast.error(
          pipelineResult.fallbackReason || "Lettura automatica non riuscita. Inserisci l'importo manualmente."
        );
      } else {
      ...
      >
        ...
        <div className="text-center">
          <p className="text-base font-black text-foreground uppercase tracking-tight">
            {hasAI ? 'Analisi AI in corso…' : 'Analisi PDF in corso…'}
          </p>
          <p className="text-xs text-muted-foreground font-medium mt-1 uppercase tracking-widest opacity-60">
            {hasAI ? 'Qwen 72B · Estrazione intelligente' : 'Estrazione automatica'}
          </p>
        </div>
      </motion.div>
    );
  ...
  if (result?.requiresManualFallback) {
    return (
      ...
      >
        ...
        {/* AI summary se disponibile */}
        {result.aiSummary && (
          <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4">
            <div className="flex items-center gap-2 mb-2">
              <Brain className="h-3.5 w-3.5 text-primary" />
              <p className="text-[10px] font-black uppercase tracking-widest text-primary">Riepilogo AI</p>
            </div>
      ...
      <div className="flex flex-col items-center justify-center gap-6 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-3 mt-2">
          {[
            ...
            { icon: hasAI ? Brain : Zap, label: hasAI ? 'AI Qwen 72B' : 'OCR automatico' },
            ...
            <span key={label} className={cn(
              ...
              label.includes('AI') || label.includes('Qwen')
                ? "border-primary/30 bg-primary/10 text-primary/80"
                : "bg-white/5 border-white/8 text-muted-foreground/60"
            )}>
